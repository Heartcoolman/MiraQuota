...
| Mirasim 版本 | `/v1/limits` 端点在 v0.0.220 上验证可用；更早版本没有该端点时，自动退回 relay 帧的百分比口径 |
...
`ANTHROPIC_BASE_URL` 使用的端口），对本机免认证，返回 `windows[].{name, used, budget, reset_at}`
...
无需反推。美元数字仍需折算：额度点单价 = 窗口内账本支出 ÷ 已用点数，取已用点数最多的窗口反推，
两个窗口共用同一单价（5h 与 7d 各自推出的单价相差约 1.2%，约合 $0.0047/点，即 1 点 ≈ 0.47 美分）。
已用点数不足 200 时不走该反推，沿用下述回归标定。

面板上 `$54.6 / $224` 的分母是该窗口自身的回归标定满额（端点可读时优先使用；
全局单价 × 预算点数仅作兜底并标注低置信）。`24.3%` 是点数比值，与美元无关；
分子 = 分母 × 百分比，三个数字在卡面上保持自洽。本机账本支出另列一行。

**退路：由百分比反推满额。** `/v1/limits` 取不到时（旧版 Mirasim 无该端点）走回归标定。
两条路径相互独立，`--doctor` 会并列输出两者，分歧超过 15% 时告警；实测差 1.9%（5h）与 0.0%（7d）。

窗口内额度百分比与 API 等价支出近似线性，满额由「一段时间内的支出 ÷ 同期百分比增量」反推，
样本越多越准。百分比分辨率只有 0.1%，因此支出与百分比增量**两侧都要挂起**，
待对面也出现非零变化时才计为一次完整观测：

- 只挂起支出（百分比不动时累积美元）：逐对丢弃零增量样本，会系统性低估满额约 60%。
...
逐对隐含满额两端各 10% 的样本会被裁掉，以剔除账号池共享造成的百分比跳变。
置信度按标定覆盖的百分比跨度与观测数判定，未达高置信时数值前加 `~` 标记。
...
整数与小数按整帧证据判定：窗口与历史缓冲的全部取值落在 (0,1] 才按小数换算——
逐值判断区分不了「真实的 0.4%」与「小数的 0.4」，而历史缓冲中重置前大于 1 的旧值足以判定刻度。
